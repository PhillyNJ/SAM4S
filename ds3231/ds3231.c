...
const char pm[] = "pm";
 ...
 uint8_t ds3231_read_register(uint8_t reg){
	...
	write_packet.chip     = DS3231_ADDRESS;
	write_packet.length = 1;
	write_packet.buffer = &write_buffer;

	while (twi_master_write(TWI_CHANNEL, &write_packet) != TWI_SUCCESS){

		if (timeout++ == I2C_TIMEOUT) {
			printf("ds3231_read_register timeout \n\r");
			break;
		}
	}
	timeout = 0;

	read_packet.chip     = DS3231_ADDRESS;
	read_packet.length = 2;
	read_packet.buffer = &read_buffer;

	while (twi_master_read(TWI_CHANNEL, &read_packet) != TWI_SUCCESS) {
 ...
 void ds3231_setDateTime(uint16_t year, uint8_t month, uint8_t day, uint8_t hour, uint8_t minute, uint8_t second){
	 ...
	 write_buffer[8] =	DS3231_REG_TIME;
	  ...
	  write_packet.chip     = DS3231_ADDRESS;
	  write_packet.length = 9;
	  write_packet.buffer = &write_buffer;

	  while (twi_master_write(TWI_CHANNEL, &write_packet) != TWI_SUCCESS){

		  if (timeout++ == I2C_TIMEOUT) {
			  printf("ds3231_setDateTime timeout \n\r");
			  break;
		  }
	  }
