 ...
 */ 
 ...
 void ds1302_init(){
	...
	PIOA->PIO_OER = CE | SCLK; 
	PIOA->PIO_OWER = CE | SCLK;
...
{	 
	...
	for (int i = 0; i < 8; ++i)
	{
		currentBit = (PIOA->PIO_PDSR & DATA) > 0;			
		value |= (currentBit << i);					
	    PIOA->PIO_SODR = SCLK;// set		
		delay_us(1);			
	    PIOA->PIO_CODR = SCLK; //clear		
		delay_us(1);
	}	
...
void ds1302_set_data_port_direction(direction_t dir){
	...
	if(dir == INPUT){
	  ...
	  PIOA->PIO_PER = DATA;
		...
		PIOA->PIO_OER = CE | DATA; 
		PIOA->PIO_OWER = CE | DATA;
	}
